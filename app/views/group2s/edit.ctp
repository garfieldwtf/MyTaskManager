...
<?php echo $form->create('Group2',array('url'=>array(isset($curgroup['Group']['name'])?$curgroup['Group']['name']:'')));?>
	<fieldset>
 		<legend><?php echo sprintf(__('Edit %s',true),__('Group',true));?></legend>
	<?php
		echo $form->input('id');
    ...
    <?php 
        ...
        echo "&nbsp;";
        echo $curLink->back($html->link(sprintf(__('Go Back to "%s"', true),sprintf(__('%s List',true),__('Group',true))), array('controller' => 'group2s', 'action' => 'index',isset($curgroup['Group']['name'])?$curgroup['Group']['name']:'')));
        echo $form->end();
    ?>
</div>
